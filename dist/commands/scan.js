...
export default class Scan extends Command {
    ...
    _getInputSourceOptions(args, flags, options) {
        if (flags.githubRepo) {
            ...
            this.error('No input source specified. Please provide the inputFile argument or use the --githubRepo flag.', { exit: 1 });
        }
    }
    /**
    ...
    async _runBatchMode(flags, args, baseOptions) {
        ...
        for (let batchNum = resumeFromBatch; batchNum <= totalBatches; batchNum++) {
            ...
            logger.info(`DEBUG: Batch calculation - startUrl=${startUrl}, batchNum=${batchNum}, batchSize=${batchSize}, range=${range}`);
            logger.info(`DEBUG: Base options - range=${baseOptions.range}, numUrls=${baseOptions.numUrls}`);
