...
export async function prebidExplorer(options) {
    ...
    try {
        ...
        if (options.range && urlSourceType !== 'GitHub' && urlSourceType !== 'PrebidDatabase') {
            ...
            logger.info(`Range ${options.range} already applied during database query. Skipping duplicate range filtering.`);
            filteringTracer.recordRangeFiltering(allUrls.length, allUrls.length, options.range);
        }
        if (allUrls.length === 0) {
        ...
        if (options.forceReprocess) {
            ...
            if (options.range) {
                ...
                logger.info(`Range filtering complete: ${originalCount} URLs in range ${options.range}, ${allUrls.length} unprocessed, ${urlsSkippedProcessed} skipped`);
            }
            else {
        ...
        const isRecoverableError = error instanceof Error && (error.message.includes('Retry cluster exceeded idle timeout') ||
