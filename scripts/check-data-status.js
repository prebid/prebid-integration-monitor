...
function checkDatabaseStatus() {
  ...
  try {
    ...
    try {
      const result = execSync('sqlite3 data/url-tracker.db "SELECT COUNT(*) FROM processed_urls;"', { encoding: 'utf8' }).trim();
      log(`🔢 Total processed URLs: ${parseInt(result).toLocaleString()}`, 'green');
    } catch (e) {
...
function checkErrorFiles() {
  ...
  log(`\n📊 Total error URLs tracked: ${totalErrorUrls.toLocaleString()}`, 'cyan');
}
...
function checkBatchProgress() {
  ...
  const progressFiles = readdirSync('.').filter(f => f.startsWith('batch-progress-') && f.endsWith('.json'));
  ...
  for (const file of progressFiles) {
    try {
      ...
      if (rangeMatch) {
        ...
        log(`📊 Range ${startUrl.toLocaleString()}-${endUrl.toLocaleString()} (${totalUrls.toLocaleString()} URLs):`, 'blue');
        log(`   • Completed batches: ${content.completedBatches?.length || 0}`, 'green');
        log(`   • Failed batches: ${content.failedBatches?.length || 0}`, content.failedBatches?.length > 0 ? 'red' : 'green');
        
        if (content.startTime) {
          log(`   • Started: ${new Date(content.startTime).toLocaleString()}`, 'magenta');
        }
        
        if (content.completedBatches && content.completedBatches.length > 0) {
          const lastBatch = content.completedBatches[content.completedBatches.length - 1];
          log(`   • Last completed: ${new Date(lastBatch.completedAt).toLocaleString()}`, 'magenta');
...
function checkTodaysActivity() {
  ...
  const logDirs = readdirSync('.').filter(d => d.startsWith('logs-batch-') && 
...
function provideSuggestions() {
  ...
  log('   • Example: --startUrl=15001 --totalUrls=5000 --batchSize=250', 'green');
  
  log('\n📊 To check what\'s been processed:', 'cyan');
  log('   • sqlite3 data/url-tracker.db "SELECT COUNT(*) FROM processed_urls;"', 'green');
