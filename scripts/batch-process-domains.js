...
class BatchProcessor {
  ...
  async processBatch(batchNumber) {
    ...
    return new Promise((resolve, reject) => {
      const args = [
        ...
        '--skipProcessed',
        '--logDir',
        `logs-batch-${batchNumber}`,
        '--chunkSize',
        '50', // Process in smaller chunks for stability
        '--verbose',
      ];
      ...
      child.stdout.on('data', (data) => {
        ...
        lines.forEach((line) => {
          if (
            line.includes('Attempting to process URL:') ||
      ...
      child.on('close', (code) => {
        ...
        if (code === 0) {
          console.log(
            `✅ Batch ${batchNumber} completed successfully in ${duration.toFixed(1)}s`
          );
          ...
          console.error(
            `❌ Batch ${batchNumber} failed with exit code ${code}`
          );
          ...
          reject(
            new Error(`Batch ${batchNumber} failed with exit code ${code}`)
          );
        }
      });
      ...
      child.on('error', (error) => {
        console.error(
          `❌ Failed to start batch ${batchNumber}:`,
          error.message
        );
        reject(error);
      });
  ...
  async run() {
    ...
    if (this.progress.lastCompletedBatch > 0) {
      ...
      console.log(
        `✅ Already processed: ${this.progress.totalProcessed} domains`
      );
      console.log(
        `⏳ Remaining: ${this.domains.length - this.progress.totalProcessed} domains`
      );
    }
    ...
    for (let batch = this.currentBatch; batch <= this.totalBatches; batch++) {
      try {
        ...
        const progressPercent = (
          (processedSoFar / this.domains.length) *
          100
        ).toFixed(1);
        console.log(
          `📈 Overall progress: ${processedSoFar}/${this.domains.length} (${progressPercent}%)`
        );
