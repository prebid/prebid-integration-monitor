...
- **Known Issue 1 (Resolved/Changed):** Previously, running with `ts-node` directly sometimes suppressed console output with OpenTelemetry. The new `npm run dev` script executes `node ./bin/dev.js`, which is the oclif-provided development entry point. This standardizes development execution and might behave differently regarding console output. Test to see if console output is as expected. If issues persist, ensure your OpenTelemetry console exporter is correctly configured and not being inadvertently silenced.
