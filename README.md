...
- `--outputDir <value>`: Specifies the directory where scan results (JSON files) will be saved.
  ...
  - Results are saved in a subdirectory structure like `store/Mmm-YYYY/YYYY-MM-DD.json` (e.g., `store/Apr-2025/2025-04-02.json`).
...
In addition to general logs, the `scan` command also logs URLs that encounter specific issues into files within the `errors/` directory. This helps in quickly identifying and isolating problematic URLs. These files are appended to with each run, allowing for accumulation of error data over time.

- `errors/no_prebid.txt`: Contains URLs where no Prebid.js integration or other specified ad technologies were detected.
- `errors/navigation_errors.txt`: Contains URLs that resulted in navigation or network-related errors (e.g., timeouts, DNS resolution failures, connection refused).
- `errors/error_processing.txt`: Contains URLs that failed during page processing for other reasons (e.g., script errors during data extraction).
