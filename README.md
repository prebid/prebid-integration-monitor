...
- `store/`: Default directory for storing primary scan data (e.g., Prebid JSON results).
- `errors/`: Default directory for storing lists of URLs that resulted in specific error conditions during scans.
- `logs/`: Default directory for application log files (`app.log`, `error.log`).
...
- `--monitor`: Enables `puppeteer-cluster`'s web monitoring interface (available at `http://localhost:21337` by default) when `puppeteerType=cluster`.
  - Default: `false`
- `--outputDir <value>`: While this flag exists, the primary Prebid JSON scan results are **always** saved to the `store/` directory (specifically `store/<Mmm-yyyy>/<yyyy-mm-dd>.json`). This flag might be used for other types of outputs in the future or by other commands.
  - Default: `store`
...
The application stores various types of output data in specific locations, primarily within the project root. These paths are generally fixed for their described purposes:

- **Successfully Scanned Prebid Data:**

  - Location: `store/<Mmm-yyyy>/<yyyy-mm-dd>.json` (e.g., `store/Apr-2025/2025-04-18.json`)
  - Description: Contains JSON arrays of `PageData` objects for URLs where Prebid.js (and potentially other ad tech) information was successfully extracted.
  - Behavior: Data is appended to the file if it already exists for the current day; otherwise, a new file is created.

- **URLs with No Prebid Information:**

  - Location: `errors/no_prebid.txt`
  - Description: A list of URLs (one per line) where the scan completed but no Prebid.js specific data was found.

- **URLs with Navigation Errors:**

  - Location: `errors/navigation_errors.txt`
  - Description: A list of URLs that resulted in navigation errors during the scan (e.g., "ENOTFOUND", "ERR_NAME_NOT_RESOLVED", DNS issues).

- **URLs with Other Processing Errors:**

  - Location: `errors/error_processing.txt`
  - Description: A list of URLs that failed during processing due to other errors not covered by navigation or no-data (e.g., unexpected script errors, timeouts not caught as navigation errors).

- **Inspection Data (from `inspect` command):**

  - Location: Default is `store/inspect/`, but configurable via `--output-dir` flag of the `inspect` command.
  - Description: Stores HTTP request/response data in JSON or HAR format.

- **Log Files:**
  - Location: `logs/` (configurable via `--logDir` flag of the `scan` command).
