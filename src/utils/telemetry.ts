...
export function initializeTelemetry(
  serviceName = 'prebid-integration-monitor'
) {
...
export class URLFilteringTracer {
  ...
  recordRangeFiltering(
    ...
    this.logger.info('🔍 TRACE [URL_FILTERING] Range applied', {
      ...
      filtered,
      filterPercentage:
        beforeCount > 0 ? ((filtered / beforeCount) * 100).toFixed(1) : '0',
      timestamp: new Date().toISOString(),
    });

    if (filtered === beforeCount) {
      this.logger.warn(
        '🔍 TRACE [URL_FILTERING] ⚠️  RANGE FILTERED ALL URLS!',
        {
          traceId: this.traceId,
          range,
          beforeCount,
  ...
  recordProcessedFiltering(
    beforeCount: number,
    afterCount: number,
    skipped: number
  ) {
    const skipPercentage =
      beforeCount > 0 ? ((skipped / beforeCount) * 100).toFixed(1) : '0';
    ...
    if (skipped === beforeCount && beforeCount > 0) {
      this.logger.warn(
        '🔍 TRACE [URL_FILTERING] ⚠️  ALL URLS ALREADY PROCESSED!',
        {
          traceId: this.traceId,
          beforeCount,
          skipped,
  ...
  recordDomainFiltering(beforeCount: number, afterCount: number) {
    ...
    if (filtered === beforeCount) {
      this.logger.error(
        '🔍 TRACE [URL_FILTERING] ❌ DOMAIN FILTERING REMOVED ALL URLS!',
        {
          traceId: this.traceId,
          beforeCount,
...
export class BatchProcessingTracer {
  ...
  recordUrlCounts(
    ...
    if (processed === 0 && skipped === 0) {
      this.logger.error(
        '🔍 TRACE [BATCH_PROCESSING] ❌ NO URLS PROCESSED OR SKIPPED!',
        {
          traceId: this.traceId,
          issue:
            'Batch completed but no URLs were processed or skipped - possible system issue',
        }
      );
    }
  }
  ...
  recordHttpRequest(url: string, statusCode?: number, contentLength?: number) {
    ...
    if (statusCode && statusCode >= 400) {
      this.logger.error('🔍 TRACE [GITHUB_FETCH] ❌ HTTP ERROR!', {
        ...
        statusCode,
        issue:
          statusCode === 404 ? 'File not found' : `HTTP error ${statusCode}`,
      });
    }
  }

  recordParsingResults(
    rawLines: number,
    validUrls: number,
    ...
    this.logger.info('🔍 TRACE [GITHUB_FETCH] Content parsed', {
      ...
      duplicatesRemoved,
      parseSuccessRate:
        rawLines > 0 ? ((validUrls / rawLines) * 100).toFixed(1) : '0',
      timestamp: new Date().toISOString(),
    });
    ...
    if (rawLines > 0 && validUrls === 0) {
      this.logger.error('🔍 TRACE [GITHUB_FETCH] ❌ NO VALID URLS FOUND!', {
        ...
        rawLines,
        issue:
          'File has content but no valid URLs extracted - check file format',
      });
    }
  }
  ...
  finish(finalUrlCount: number) {
    ...
    if (finalUrlCount === 0) {
      this.logger.error('🔍 TRACE [GITHUB_FETCH] ❌ ZERO URLS RETURNED!', {
        traceId: this.traceId,
        issue:
          'GitHub fetch completed but returned no URLs - check source and format',
      });
    }
  }
}
      ...
      if (result instanceof Promise) {
        return result
          .then((res) => {
            ...
            if (logger) {
              logger.debug(`🔍 TRACE [${name}] Completed`, {
                traceId,
                durationMs: duration,
              });
            }
