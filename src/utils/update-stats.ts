...
async function readAndParseFiles(
  ...
  try {
    ...
    for (const entry of outputEntries) {
      if (entry.isDirectory() && monthDirRegex.test(entry.name)) {
        ...
        try {
          ...
          for (const file of files) {
            if (path.extname(file).toLowerCase() === '.json') {
              ...
              } catch (parseOrReadFileError) { // Errors from readJsonFile are AppErrors
        ...
        } catch (monthDirReadError) { // Errors from readDirectory are AppErrors
          logger.instance.warn(
  ...
  } catch (baseDirError: unknown) { // Could be AppError from readDirectory or other fs error
...
async function updateAndCleanStats(): Promise<void> {
  try {
    ...
    if (err instanceof AppError) { // If it's already an AppError, log its details
      logger.instance.error(
    ...
    } else if (err instanceof Error) { // For other native errors
      logger.instance.error(
    ...
    } else { // For unknown throw types
      logger.instance.error(
        'A critical and unknown error occurred in updateAndCleanStats.', { error: err }
      );
    }
