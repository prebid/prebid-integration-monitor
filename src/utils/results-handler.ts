...
export function updateInputFile(
  ...
  try {
    ...
    for (const taskResult of taskResults) {
      ...
      if (taskResult && taskResult.type === 'success' && taskResult.data.url && urlsInCurrentProcessingScope.includes(taskResult.data.url)) {
    ...
    if (fs.existsSync(inputFilepath)) {
      ...
      const currentScopeSet = new Set(urlsInCurrentProcessingScope.map(url => url.trim()));
