 ...
 */
export function processAndLogTaskResults(taskResults: TaskResult[], logger: WinstonLogger): PageData[] {
    ...
    for (const taskResult of taskResults) {
        ...
        switch (type) {
            case 'success':
                ...
                logger.info(`SUCCESS: Data extracted for ${taskResult.data.url}`, { url: taskResult.data.url, version: taskResult.data.prebidInstances?.[0]?.version });
                successfulResults.push(taskResult.data);
                break;
            case 'no_data':
                // Type guard ensures taskResult is TaskResultNoData here
                logger.warn(`NO_DATA: No relevant ad tech data found for ${taskResult.url}`, { url: taskResult.url });
                break;
            case 'error':
                // Type guard ensures taskResult is TaskResultError here
                logger.error(`ERROR: Processing failed for ${taskResult.url} - ${taskResult.error}`, { url: taskResult.url, errorDetails: taskResult.error });
                break;
            default:
                ...
                logger.warn(`Unknown task result type encountered: '${exhaustiveCheck}'`, { result: taskResult });
        }
    }
    logger.info(`Finished processing task results. ${successfulResults.length} successful extractions.`);
    return successfulResults;
}
 ...
 */
export function writeResultsToFile(resultsToSave: PageData[], baseOutputDir: string, logger: WinstonLogger): void {
    ...
    try {
        ...
        const monthDir = path.join(baseOutputDir, `${year}-${monthPadded}-${monthShort}`);
        ...
        logger.info(`Successfully wrote ${resultsToSave.length} results to ${filePath}`);
    ...
    try {
        ...
        if (fs.existsSync(inputFilepath)) {
            ...
            finalUrlsToWrite = originalUrls.filter(url => {
                if (currentScopeSet.has(url)) { // If URL was in current scope
