...
export const processPageTask = async ({
  ...
  try {
    ...
    const extractedPageData: PageData = await page.evaluate(
      ...
      async (pbjsTimeoutMs, pbjsIntervalMs): Promise<PageData> => {
        ...
        const getPbjsInstanceData = async (globalVarName: string) => {
          ...
          while (elapsedTime < pbjsTimeoutMs) { // Use passed-in pbjsTimeoutMs
    ...
    const originalMessage = pageError.message || 'Unknown error during page processing';
    ...
    logger.error(
      `Error processing ${trimmedUrl}: ${originalMessage}`,
      {
        url: trimmedUrl,
        errorCode,
