...
export default class Inspect extends Command {
  ...
  static override flags = {
    ...
    filename: Flags.string({
     ...
     logDir: Flags.string({ // Added logDir flag
  ...
  public async run(): Promise<void> {
    ...
    if (flags.filename) {
      logger.info(`Custom filename specified: ${flags.filename}.${flags.format}`);
    }
    ...
    try {
      ...
      logger.info(`Received response: ${response.status} ${response.statusText}`);
      ...
      const suggestions = ['Check the URL and network connectivity.', 'Verify file system permissions for the output directory.'];

      if (error instanceof AppError) {
        currentLogger.error(`AppError during inspection of ${args.url}: ${error.message}`, {
        ...
        currentLogger.error(`Error during inspection of ${args.url}: ${error.message}`, {
          stack: error.stack,
        });
        userMessage = `Error during inspection of ${args.url}: ${error.message}`;
      } else {
        currentLogger.error(`An unknown error occurred during inspection of ${args.url}.`, { errorDetail: JSON.stringify(error, null, 2) });
      }
