...
describe('Progress Tracking and Statistics Tests', () => {
  ...
  describe('Comprehensive Statistics Logging', () => {
    it('should log accurate scan summary with all statistics', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringMatching(
          /📁 Output file created: store\/Jun-2025\/\d{4}-\d{2}-\d{2}\.json/
        )
      );
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        '   • Continue with next range: --range "1001-2000"'
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        '   • Reprocess this range: --resetTracking'
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        '   • Process without deduplication: remove --skipProcessed'
      );
    });
    ...
    it('should provide helpful guidance when no data is extracted', async () => {
      const testUrls = Array.from(
        { length: 5 },
        (_, i) => `https://no-data-test${i + 1}.com`
      );
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        '💡 No data was extracted because:'
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringMatching(
          /• \d+ URLs were already processed \(use --resetTracking to reprocess\)/
        )
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        '📁 No output file created (no successful extractions)'
      );
    });
  });
  ...
  describe('Progress Tracking During Processing', () => {
    it('should track progress accurately in chunk processing mode', async () => {
      const testUrls = Array.from(
        { length: 8 },
        (_, i) => `https://chunk-progress${i + 1}.com`
      );
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        'Processing chunk 1 of 3: URLs 1-3'
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        'Processing chunk 2 of 3: URLs 4-6'
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        'Processing chunk 3 of 3: URLs 7-8'
      );

      expect(mockLogger.info).toHaveBeenCalledWith(
        'Finished processing chunk 1 of 3.'
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        'Finished processing chunk 2 of 3.'
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        'Finished processing chunk 3 of 3.'
      );
    });
    ...
    it('should track cluster queue and idle operations', async () => {
      const testUrls = Array.from(
        { length: 6 },
        (_, i) => `https://cluster-progress${i + 1}.com`
      );
  ...
  describe('URL Range and Filtering Statistics', () => {
    it('should accurately track URL counts through filtering pipeline', async () => {
      const originalUrls = Array.from(
        { length: 50 },
        (_, i) => `https://pipeline-test${i + 1}.com`
      );
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        'Pre-filtering URLs for domain validity...'
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringMatching(
          /Domain pre-filtering complete: \d+ total, \d+ valid, \d+ filtered out/
        )
      );

      expect(mockLogger.info).toHaveBeenCalledWith(
        'Filtering out previously processed URLs...'
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringMatching(
          /URL filtering complete: \d+ total, \d+ unprocessed, \d+ skipped/
        )
      );
    });
    ...
    it('should handle edge case where all URLs are filtered out', async () => {
      const testUrls = Array.from(
        { length: 10 },
        (_, i) => `https://filtered-out${i + 1}.com`
      );
  ...
  describe('Database Statistics Integration', () => {
    it('should update URL tracker and log database statistics', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('URL tracker statistics:', {
        success: 100,
        no_data: 20,
        error: 10,
      });
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        '💾 Database total: 142 processed URLs'
      );
    });
