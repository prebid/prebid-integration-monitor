...
describe('GitHub Range Integration Tests', () => {
  ...
  describe('GitHub Range Processing Pipeline', () => {
    it('should handle small ranges correctly without double application', async () => {
      ...
      const mockGitHubUrl = 'https://github.com/test/repo/blob/master/domains.txt';
      
      // Mock fetch to return content simulating 1M domains
      const mockDomains = Array.from({ length: 1000 }, (_, i) => `domain${i + 1}.com`);
      ...
      vi.mocked(fetch.default).mockResolvedValue({
        ...
        headers: { get: (key: string) => key === 'content-length' ? '10000' : null }
      } as any);
    ...
    it('should handle large range positions correctly - THE BUG SCENARIO', async () => {
      ...
      const mockGitHubUrl = 'https://github.com/test/repo/blob/master/top-1m-domains.txt';
      
      // Create mock content with 500k+ lines
      const totalLines = 500010;
      const mockContent = Array.from({ length: totalLines }, (_, i) => `domain${i + 1}.com`).join('\n');
      
      const fetch = await import('node-fetch');
      vi.mocked(fetch.default).mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(mockContent),
        headers: { get: (key: string) => key === 'content-length' ? String(mockContent.length) : null }
      } as any);
    ...
    it('should detect when GitHub range and manual range would conflict - BUG DETECTION', () => {
      ...
      const githubExtractedUrls = ['https://domain500000.com', 'https://domain500001.com'];
      ...
      const shouldSkipManualRange = sourceType === 'GitHub' && !!manualRangeString;
  ...
  describe('Edge Cases That Caused Silent Failures', () => {
    it('should handle boundary conditions correctly', async () => {
      const testCases = [
        ...
        { range: '999-1001', expectedCount: 3, description: 'range crossing boundaries' },
        { range: '1000-1000', expectedCount: 1, description: 'single URL at boundary' }
      ];

      for (const testCase of testCases) {
        const mockDomains = Array.from({ length: 1000 }, (_, i) => `edge${i + 1}.com`);
        const mockContent = mockDomains.join('\n');
