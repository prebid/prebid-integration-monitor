...
interface ClusterConfig {
  concurrency:
    | typeof Cluster.CONCURRENCY_CONTEXT
    | typeof Cluster.CONCURRENCY_PAGE
    | typeof Cluster.CONCURRENCY_BROWSER; // More specific type
 ...
 */
const getPbjsVersionWithWait = async (
    ...
    async (timeoutMs, intervalMs) => {
      ...
      while (elapsedTime < timeoutMs) {
        ...
        if (
          ...
          (window as any).pbjs.version.length > 0
        ) {
      ...
      throw new PbjsVersionTimeoutError(
        'pbjs.version not found or invalid after timeout',
      );
    },
 ...
 */
const getPbjsVersionFromPage = async (
  page: Page,
  url: string,
): Promise<string | null> => {
  try {
    ...
    logError(
      url,
      `Failed to get pbjs.version from main page (or timed out): ${e.message}`,
      e,
      'getting pbjs.version from main page',
    );
    return null;
  }
};
 ...
 */
const getPbjsVersionFromFrames = async (
  page: Page,
  url: string,
): Promise<string | null> => {
  ...
  if (frames.length > 1) {
    for (const frame of frames) {
      ...
      try {
        ...
        if (frameVersion) {
          logger.info(
            `PBJS Version (found in frame ${frameUrl}): ${frameVersion}`,
            { url, frameUrl },
          );
        ...
        logError(
          url,
          `Error evaluating frame ${currentFrameUrl} or timed out: ${frameError.message}`,
          frameError,
          'evaluating frame for pbjs.version',
        );
      }
    }
  }
...
const clusterSearch = async (): Promise<void> => {
  const cluster: Cluster<string, any> = await Cluster.launch({
    ...
    async ({ page, data: url }: { page: Page; data: string }) => {
      ...
      try {
        ...
        if (version) {
          ...
          logger.info(
            `PBJS Version: Not found on main page. Checking frames...`,
            { url },
          );
          ...
          if (!versionFromFrames) {
            ...
            logger.warn(
              `PBJS Version: Not found in any (accessible/non-timed-out) frame after waiting.`,
              { url },
            );
          }
        ...
        logError(
          url,
          'Navigation or task processing error',
          e,
          'processing URL in cluster task',
        );
      }
    },
  );
...
clusterSearch().catch((error: Error) => {
