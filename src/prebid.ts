...
export async function prebidExplorer(
  ...
  (puppeteer as { use: (plugin: unknown) => void }).use(StealthPlugin());
  ...
  const blockResourcesFactory =
    (BlockResourcesModule as { default: unknown }).default ||
    BlockResourcesModule;
  const blockResourcesPluginInstance = blockResourcesFactory({
    blockedTypes: resourcesToBlock,
  });
  // Cast puppeteer to any before calling use
  (puppeteer as { use: (plugin: unknown) => void }).use(
    blockResourcesPluginInstance,
  );
  logger.info(
  ...
  if (chunkSize > 0) {
    ...
    for (let i = 0; i < urlsToProcess.length; i += chunkSize) {
      ...
      if (options.puppeteerType === 'cluster') {
        ...
        try {
          ...
          if (cluster && !(cluster as { isClosed: () => boolean }).isClosed())
            await (cluster as { close: () => Promise<void> }).close(); // Ensure cluster is closed on error
        }
      } else {
    ...
    if (options.puppeteerType === 'cluster') {
      ...
      try {
        ...
        // Cast cluster to any before calling isClosed and close
        if (cluster && !(cluster as { isClosed: () => boolean }).isClosed())
          await (cluster as { close: () => Promise<void> }).close();
      }
    } else {
      // 'vanilla' Puppeteer
      let browser: Browser | null = null;
